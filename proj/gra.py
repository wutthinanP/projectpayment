...
ccourse_id = ['01355101', '01355102', '01417111', '01418111', '01418112', '01418141', '01999021', '01999111', '02999144']
ccourse_details = {'01355101': ('English for Everyday Life', 3),
...
course_id = ["Soft Crust Pizza","Crust Pizza"]  #Pizzadough_list 
...
course_details={'Soft Crust Pizza':150,'Crust Pizza':150,'Mozzarella Cheese':25,'Bacon Bits':40,'Mushrooms':15,'Prawn':20,'Seafood':40,'Tomato':10
...
def updateGPA():
    ...
    # *** หาหน่วยกิตรวม จาก List added_credit  โดยใช้ sum ของ numpy
    sum_credits = np.sum(added_credit)
    # *** หาเกรดรวมจาก List added_credit*added_grade  โดยใช้ sum ของ numpy
    sum_grades = np.sum(np.multiply(added_credit, added_grade))
    # *** คำนวณหาร GPA
    GPA = sum_grades / sum_credits

    # *** แสดงค่าจำนวนหน่วยกิต ใน lbl_Credits
    lbl_Credits.config(text="จำนวนหน่วยกิต: \n" + str(sum_credits))
    # *** แสดง GPA ใน lbl_GPA
    lbl_GPA.config(text="ผลการเรียน: \n" + str('{:.2f}'.format(GPA)))
...
def click(action, root, table):
    ...
    if action == 'add':
        ...
        # ถ้าเลือกรหัสวิชาไม่ซ้ำ สร้างรายการใหม่ในตาราง และเพิ่มรายการวิชา หน่วยกิต และผลการเรียน
            ...
            # *** สร้างตัวแปร List เพื่อรอ Insert ลง Tree
            content = [course_id, course_name, credit, grade]
            # *** Insert ลง Tree
            table.insert('', tk.END, values=content)
            added_course.append(course_id)

            # *** บันทึกลง List added_course เพื่อเก็บข้อมูลวิชาที่ลงทะเบียนแล้ว
            if float(grade) >= 0.0:
                # *** บันทึกลง List added_credit เพื่อการคำนวณหน่วยกิตรวม
                added_credit.append(int(credit))
                # *** บันทึกลง List added_grade เพื่อการคำนวณเกรด
                added_grade.append(float(grade))

            # ถ้าเป็นเกรดที่ไม่มีค่าเพื่อคำนวณเช่น I
            else:
                # *** บันทึก 0 ลง List added_credit
                added_credit.append(0)
                # *** บันทึก 0 ลง List added_grade
                added_grade.append(0)

            # เสร็จแล้วเรียก function updateGPA()
            updateGPA()
    ...
    # กรณีที่คลิกปุ่ม delete
        ...
        # *** วัน Loop อ่านค่า item จาก Tree ที่เลือก
        for select_item in table.selection():
            ...
            # ทำการค้าหาค่าจาก List added_course
            idx = added_course.index(id)
            if idx >= 0:
                answer = askyesno(title='ยืนยันการลบ',
                                  message='คุณต้องการลบรายการนี้หรือไม่?')
                if answer:
                    # ***  เอาค่า idx ออกจาก List added_course
                    added_course.pop(idx)
                    # ***  เอาค่า idx ออกจาก List added_credit
                    added_credit.pop(idx)
                    # ***  เอาค่า idx ออกจาก List added_grade
                    added_grade.pop(idx)
...
def create_button_frame(container, table):
    ...
    btn_delete = ttk.Button(frame, text='Delete', command=lambda: click('delete', container, table)).grid(column=0,
                                                                                                          row=1)
